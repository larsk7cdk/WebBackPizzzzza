...
namespace WebBackPizzzzza.web.Services
{
    public class CultureProviderResolverService : RequestCultureProvider
    {
        ...
        {
            if (GetCultureFromQueryString(httpContext, out var culture))
                return new ProviderCultureResult(culture, culture);

            if (GetCultureFromCookie(httpContext, out culture))
                return new ProviderCultureResult(culture, culture);

            if (GetCultureFromSession(httpContext, out culture))
                return new ProviderCultureResult(culture, culture);
        ...
        public static string ParseCookieValue(string value)
        {
            ...
            if (cultureName != null && uiCultureName == null) uiCultureName = cultureName;
